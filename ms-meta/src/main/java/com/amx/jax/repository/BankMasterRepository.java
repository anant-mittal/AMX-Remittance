...
public interface BankMasterRepository extends CrudRepository<BankMasterMdlv1, BigDecimal> {
	...
	public List<BankMasterMdlv1> findBybankCountryIdAndRecordStatusOrderByBankShortNameAsc(BigDecimal bankCountryId,
			String isActive);
	...
	@Query(value = "select distinct T1.* from ex_bank_master T1 inner join V_BENE_SERVICE_CURRENCY T2 on t1.bank_country_id = t2.bene_country_id"
			+ " where t1.bank_country_id=?1 and t2.currency_id = ?2 and T1.isactive='Y'", nativeQuery = true)
