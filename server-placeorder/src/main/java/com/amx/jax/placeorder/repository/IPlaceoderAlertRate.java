...
public interface IPlaceoderAlertRate  extends JpaRepository<PlaceOrder, Serializable> {
	...
	@Query("select p from   PlaceOrder p  where p.countryId =:countryId and  p.currencyId =:currencyId and p.bankId =:bankId  and p.targetExchangeRate <=:targetExchangeRate and  (to_date(p.notificationDate,'dd-mm-yy') < to_date(SYSDATE,'dd-mm-YY') or p.notificationDate IS NULL) ")
