...
public class PushNotifyClient implements IPushNotifyService {
	...
	public PushMessage sendDirect(PushMessage msg) throws PostManException {
		...
		try {
			return restService.ajax(appConfig.getPostmapURL()).path(PostManUrls.NOTIFY_PUSH)
					.post(new HttpEntity<PushMessage>(msg)).as(PushMessage.class);
		} catch (Exception e) {
	 ...
	 */
	public PostManResponse send(PushMessage msg) throws PostManException {
	...
	public PostManResponse subscribe(String token, String topic) throws PostManException {
		...
		try {
			return restService.ajax(appConfig.getPostmapURL()).path(PostManUrls.NOTIFY_PUSH_SUBSCRIBE)
					.queryParam(PARAM_TOKEN, token).pathParam(PARAM_TOPIC, topic).post().as(PostManResponse.class);
		} catch (Exception e) {
			throw new PostManException(e);
		}
