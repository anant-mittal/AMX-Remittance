...
public class HomesendGate
{
	...
	/*public HomesendGate(String api_login, String api_password, String vendor_id, String api_url, OwsParamRespcodeRepository owsParamRespcodeRepository)
	{
      ...
      public Quotation_Call_Response getQuotation(BigDecimal settlement_amount,
							...
							String beneficary_reference,
							String beneficary_type,
	...
	{
		/*
		 * This is service will be used to get the following:  Rate  Commission and
		 * final mount to be charged and credited in settlement currency and destination
		 * currency  Bene Account validation � HomeSend Transaction ID We need to pass
		 * as in put the following:  Amount and currency. See the question section. 
		 * Bene and customer identification � Routing tag which is a list of possible
		 * option shared by partner. See the question section. This service will be used
		 * as first call in the transaction completion cycle and it need to be called
		...
		try
		{
			String validation_result = HomesendUtils.validate_quotation_request_input(settlement_amount,
					settlement_currency,
					destination_amount,
					destination_currency,
					destination_country_3_digit_ISO,
			...
			if (validation_result != null && validation_result.length() != 0) // Validation Error
			{
				...
				logger.info("Validation Result for transaction ( request_sequence_id= " + request_sequence_id
						+ " / customer_reference= " + customer_reference + " / beneficary_reference= "
						+ beneficary_reference + " ): /n " + validation_result);
			}
			else
			{
				customer_reference = CUSTOMER_ID_PREFIX + customer_reference;

				beneficiary_account = HomesendUtils.form_bank_account(beneficiary_account,
						is_iban_number,
						bic_indicator,
						beneficiary_bank_branch_swift_code,
						beneficiary_bank_code,
			...
			routting_tag = HomesendUtils.form_routting_tag(destination_country_3_digit_ISO,
								       remittance_mode,beneficiary_bank_branch_swift_code);
				...
				List<VendorSpecificField> vendor_specific_fields_list = HomesendUtils
						.form_account_of_charge_vsf_list(origin_country_3_digit_ISO,
								service_type,
								destination_currency,
								beneficiary_bank_branch_swift_code,
															      beneficiary_bank_code,
				...
				try
				{
					logger.info("Start calling Fee Inquiry API for transaction ( request_sequence_id= "
							+ request_sequence_id + " / customer_reference= " + customer_reference
							+ " / beneficary_reference= " + beneficary_reference + " ) ...");

					response = HomeSend_BindingStub.adviceOfCharge(request);

					endTime = System.currentTimeMillis();

					logger.info("Fee Inquiry API for transaction completed( request_sequence_id= " + request_sequence_id
							+ " / customer_reference= " + customer_reference + " / beneficary_reference= "
							+ beneficary_reference + " ) took " + ((endTime - startTime) / 1000.0) + " Seconds");
					...
					if (
						...
						local_api_response.setResponse_description("Destination currency received from partner ("
								...
								+ ") is not matching with Al-Mulla destination currency: " + destination_currency);
				...
				catch (HomeSendServiceException e)
				{
					...
					try
					{
						logger.info("Fee Inquiry API for transaction completed with expection( request_sequence_id= "
								+ request_sequence_id + " / customer_reference= " + customer_reference
								+ " / beneficary_reference= " + beneficary_reference + " ) took "
								+ ((endTime - startTime) / 1000.0) + " Seconds");

						OwsParamRespcode temp_ows_param_respcode_record = owsParamRespcodeRepository
								.findByOwsParamRespcodeKey(new OwsParamRespcodeKey(SERVICE_PROVIDER,
										e.getCode(),
										GET_QUOTATION_METHOD_IND));
						...
						local_api_response.setTechnical_details(HomesendUtils.getStackTrace(e));
					...
					{
						logger.info(HomesendUtils.getStackTrace(ex));
		...
		catch (Exception e)
		{
			...
			if (local_api_response.getResponse_code() == null)
			{
				...
				if (
					e != null && e.getMessage() != null && (e.getMessage().toUpperCase().contains("CONNECTION")
			...
			Benificiary bene_data)
	{
		...
		try
		{
			...
			routting_tag = HomesendUtils.form_routting_tag(txn_data.getDestination_country_3_digit_ISO(),
								       txn_data.getRemittance_mode(),bene_data.getBeneficiary_bank_branch_swift_code());

				service_type = HomesendUtils.form_service_type(customer_data.getCustomer_type(),
				...
				ArrayList<VendorSpecificField> vendor_specific_fields_list = HomesendUtils
						.form_remittance_vsf_list(txn_data, customer_data, bene_data, API_LOGIN);
				...
				try
				{
					logger.info("Start calling Send Remittance API for transaction ( HomeSend ID= "
							+ txn_data.getPartner_transaction_reference() + " / customer_reference= "
							+ customer_data.getCustomer_reference() + " / beneficary_reference= "
					...
					logger.info("Send Remittance API completed for transaction ( HomeSend ID= "
							+ txn_data.getPartner_transaction_reference() + " / customer_reference= "
							+ customer_data.getCustomer_reference() + " / beneficary_reference= "
							+ bene_data.getBeneficiary_reference() + "  took " + ((endTime - startTime) / 1000.0)
							+ " Seconds");
					...
					else
					{
						...
						local_api_response.setBene_bank_remittance_reference(
								HomesendUtils.get_filed_value(response.getVendorSpecificFields(),
										new UnsignedInt(HomesendUtils.getProperties("TRANSACTION_SECRET_CODE",
												PROPERTY_FILE_PATH))));
					}
				}
				catch (HomeSendServiceException e)
	...
	public Status_Call_Response get_remittance_status(String request_sequence_id, String homeSend_transaction_id)
	{
		...
		try
		{
			String validation_result = HomesendUtils.validate_status_request_input(request_sequence_id,
			...
			else
			{
				...
				try
				{
					...
					else if (response.getStatus().getRejected() != null)
					{
						OwsParamRespcode temp_ows_param_respcode_record = owsParamRespcodeRepository
								.findByOwsParamRespcodeKey(new OwsParamRespcodeKey(SERVICE_PROVIDER,
										response.getStatus().getRejected().getCode(),
					...
					try
					{
						OwsParamRespcode temp_ows_param_respcode_record = owsParamRespcodeRepository
								.findByOwsParamRespcodeKey(new OwsParamRespcodeKey(SERVICE_PROVIDER,
										e.getCode(),
										REMITTANCE_STATUS_INQ_IND));
						local_api_response.setHigh_level_response("UnKnown");
						local_api_response.setResponse_code(e.getCode());
						local_api_response.setResponse_description(e.getFaultString());
						local_api_response.setAction_ind(
								temp_ows_param_respcode_record != null ? temp_ows_param_respcode_record.getAction_ind()
				...
				if (
					...
					{
					local_api_response.setResponse_description(
							local_api_response.getHigh_level_response() + ", " + local_api_response.getResponse_code());
				}
			}
		}
		catch (Exception e)
