...
public interface PayAtBranchTrnxListRepository extends JpaRepository<PayAtBranchTrnxModel, Serializable>{
	@Query("select w from PayAtBranchTrnxModel w where w.customerId=?1 and (w.applIsActive='Y'and wireTransferStatus='NEW') and wireTransferStatus is not 'PAID'")
	public List<PayAtBranchTrnxModel> getPbTrnxList(BigDecimal customerId);
	
	@Query("select w from PayAtBranchTrnxModel w where (w.applIsActive='Y'and wireTransferStatus='NEW') order by documentDate desc")
