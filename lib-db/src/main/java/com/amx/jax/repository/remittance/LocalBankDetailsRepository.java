...
public interface LocalBankDetailsRepository extends CrudRepository<LocalBankDetailsView, Serializable>{
	...
	@Query(value = "Select Distinct(Debit_Card_Name) ,RELATIONS_ID, CARD_ID from Ex_Customer_Bank where Customer_Id = ?1 and Bank_Id = ?2 and Isactive = 'Y'", nativeQuery = true)
