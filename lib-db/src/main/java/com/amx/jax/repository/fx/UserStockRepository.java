...
public interface UserStockRepository extends CrudRepository<UserStockView, Serializable>{
	...
	@Query(value = "SELECT * FROM V_EX_STOCK WHERE ORACLE_USER=?1 AND COUNTRY_BRANCH_ID=?2 AND CURRENCY_ID=?3 AND TRUNC(LOG_DATE)=TRUNC(SYSDATE)", nativeQuery = true)
	public List<UserStockView> fetchUserStockByCurrencyDate(String userName,BigDecimal countryBranchId,BigDecimal foreignCurrencyId);
	...
	@Query(value = "SELECT * FROM V_EX_STOCK WHERE ORACLE_USER=?1 AND COUNTRY_BRANCH_ID=?2 AND TRUNC(LOG_DATE)=TRUNC(SYSDATE)", nativeQuery = true)
	public List<UserStockView> fetchUserStockByDate(String userName,BigDecimal countryBranchId);
