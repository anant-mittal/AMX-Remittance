...
public interface VTransferRepository extends CrudRepository<ViewTransfer, String> {
	...
	@Query(value="SELECT FROM TRANSFER WHERE CUSREF=:cusref AND TRUNC(DOCDAT)=TRUNC(SYSDATE) "
	...
	public List<ViewTransfer> todayTransactionCheck(
			@Param("cusref") BigDecimal cusref,
			@Param("bnfBankCode") String bnfBankCode,
			@Param("accountNo") String accountNo,
			@Param("engBnfName") String engBnfName,
			@Param("locCode") BigDecimal locCode);
