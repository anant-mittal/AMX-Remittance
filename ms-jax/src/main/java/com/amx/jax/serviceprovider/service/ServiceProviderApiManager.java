...
public class ServiceProviderApiManager {
	...
	// store xml logs
	public void fetchServiceProviderCommitLogs(ServiceProviderResponse srvPrvResp,ServiceProviderCallRequestDto serviceProviderCallRequestDto) {
		String requestXml = null;
		String responseXml = null;
		String referenceNo = null;
		BigDecimal customerReference = null;
		...
		if(serviceProviderCallRequestDto != null && srvPrvResp != null) {
			...
			if(serviceProviderCallRequestDto.getTransactionDto() != null) {
				Remittance_Call_Response remittance_Call_Response = (Remittance_Call_Response) srvPrvResp;
				if(remittance_Call_Response.getPartner_transaction_reference() != null) {
					partnerReference = remittance_Call_Response.getPartner_transaction_reference();
				}
				if(serviceProviderCallRequestDto.getTransactionDto().getRoutting_bank_code() != null) {
					bankCode = serviceProviderCallRequestDto.getTransactionDto().getRoutting_bank_code();
				}
			...
			if(srvPrvResp.getRequest_XML() != null) {
				requestXml = srvPrvResp.getRequest_XML();
				ServiceProviderLogDTO serviceProviderXmlLog = partnerTransactionManager.saveServiceProviderXMLlogData(PricerServiceConstants.COMMIT_REQUEST, requestXml, referenceNo, reqSeq, PricerServiceConstants.REQUEST, trnxType, partnerReference, customerReference, bankCode);
				if(serviceProviderXmlLog != null) {
					partnerTransactionManager.saveServiceProviderXml(serviceProviderXmlLog);
				}
			}
			if(srvPrvResp.getResponse_XML() != null) {
				responseXml = srvPrvResp.getResponse_XML();
				ServiceProviderLogDTO serviceProviderXmlLog = partnerTransactionManager.saveServiceProviderXMLlogData(PricerServiceConstants.COMMIT_RESPONSE, responseXml, referenceNo, resSeq, PricerServiceConstants.RESPONSE, trnxType, partnerReference, customerReference, bankCode);
				if(serviceProviderXmlLog != null) {
					partnerTransactionManager.saveServiceProviderXml(serviceProviderXmlLog);
				}
			}
	...
	// save the bank reference and ventaja details
	public void updateServiceProviderDetails(ServiceProviderResponse srvPrvResp,BigDecimal remittanceTransactionId) {
		String partnerReference = null;
		String additionalExternalReference = null;
		ServiceProviderResponse serviceProviderResponse = srvPrvResp;
		Remittance_Call_Response remittance_Call_Response = (Remittance_Call_Response) serviceProviderResponse;
		if(remittance_Call_Response != null) {
			if(remittance_Call_Response.getPartner_transaction_reference() != null) {
				partnerReference = remittance_Call_Response.getPartner_transaction_reference();
			}
			if(remittance_Call_Response.getAddtional_external_reference() != null) {
				additionalExternalReference = remittance_Call_Response.getAddtional_external_reference();
			}
			if(partnerReference != null || additionalExternalReference != null) {
				partnerTransactionDao.updateServiceProviderDetails(partnerReference, additionalExternalReference, remittanceTransactionId);
			}
		}
	}
