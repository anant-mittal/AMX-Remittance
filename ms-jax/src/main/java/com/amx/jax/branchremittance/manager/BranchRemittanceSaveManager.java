...
public class BranchRemittanceSaveManager {
	...
	public RemittanceResponseDto saveRemittance(BranchRemittanceRequestModel remittanceRequestModel) {
		...
		try {
			...
			List<ForeignCurrencyAdjust> currencyAdjustList = saveForeignCurrencyAdjust(remittanceRequestModel,
					collectionModel);
		...
		try {
			for (BranchApplicationDto applId : shoppingCartList) {
				RemittanceApplication remittanceApplication = remittanceApplicationRepository
						.findOne(applId.getApplicationId());
	...
	private CollectionModel saveCollect(BranchRemittanceRequestModel remittanceRequestModel) {
		...
		try {
			if (shoppingCartList != null && !shoppingCartList.isEmpty()) {
				...
				RemittanceApplication appl = remittanceApplicationRepository
						.findOne(shoppingCartList.get(0).getApplicationId());
				...
				collection.setRefoundAmount(
						remittanceRequestModel.getPaidAmount().subtract(remittanceRequestModel.getTotalTrnxAmount()));
				...
				BigDecimal documentNo = generateDocumentNumber(
						appl.getFsCountryMasterByApplicationCountryId().getCountryId(),
						appl.getFsCompanyMaster().getCompanyId(), collection.getDocumentId(),
						collection.getDocumentFinanceYear(), appl.getExCountryBranch().getBranchId());
				...
				BigDecimal declarationTotalamount = getDeclarationReportAmount(
						ConstantDocument.DECL_REPORT_FOR_TOT_AMOUNT);
	...
	public List<CollectDetailModel> saveCollectionDetail(BranchRemittanceRequestModel remittanceRequestModel,
		...
		for (RemittanceCollectionDto collectDataTable : collectionDetails) {
			...
			if (payMode.getPaymentModeCode().equalsIgnoreCase(ConstantDocument.CHEQUE)) {
				...
				collectDetails.setChequeDate(collectDataTable.getColchequeDate() == null ? new Date()
						: DateUtil.convertStringToDate(collectDataTable.getColchequeDate()));
				collectDetails.setApprovalNo(collectDataTable.getApprovalNo());
				collectDetails.setChequeBankRef(collectDataTable.getChequeBankCode());
			}
	...
	public List<ForeignCurrencyAdjust> saveForeignCurrencyAdjust(BranchRemittanceRequestModel remittanceRequestModel,
			CollectionModel collect) {
	...
	public List<RemittanceTransaction> saveRemittanceTrnx(BranchRemittanceRequestModel remittanceRequestModel,
			CollectionModel collect) {
		...
		if (shoppingCartList != null && !shoppingCartList.isEmpty()) {
			...
			for (BranchApplicationDto applDto : shoppingCartList) {
				...
				if (appl != null && appl.getIsactive().equalsIgnoreCase(ConstantDocument.Yes)) {
					...
					remitTrnx.setDocumentFinanceYr(
							appl.getExUserFinancialYearByDocumentFinanceYear().getFinancialYearID());

					List<Document> documentList = documentDao
							.getDocumnetByCode(ConstantDocument.REMITTANCE_DOCUMENT_CODE);

					if (documentList != null && !documentList.isEmpty()) {
						remitTrnx.setDocumentId(documentList.get(0));
						remitTrnx.setDocumentCode(documentList.get(0).getDocumentCode());
					} else {
						throw new GlobalException(JaxError.INVALID_REMITTANCE_DOCUMENT_CODE,
								"Document ID could not be updated in our records.");
					}
					BigDecimal documentNo = generateDocumentNumber(
							appl.getFsCountryMasterByApplicationCountryId().getCountryId(),
							appl.getFsCompanyMaster().getCompanyId(), remitTrnx.getDocumentId().getDocumentCode(),
							remitTrnx.getDocumentFinanceYear(), remitTrnx.getBranchId().getBranchId());

					if (documentNo != null && documentNo.compareTo(BigDecimal.ZERO) != 0) {
						remitTrnx.setDocumentNo(documentNo);
					} else {
						throw new GlobalException(JaxError.INVALID_REMITTANCE_DOCUMENT_NO,
								"Document Seriality  setup  not defined for Remittance.");
					}
	...
	public Map<String, RemittanceBenificiary> saveBeneTrnx(List<RemittanceTransaction> remitTrnxList) {
		...
		for (RemittanceTransaction remitTrnx : remitTrnxList) {
			...
			if (applBene != null) {
				...
				remitBeneList.put(remitTrnx.getApplicationFinanceYear() + "/" + remitTrnx.getApplicationDocumentNo(),
						remitBene);

			} else {
				throw new GlobalException(JaxError.NO_RECORD_FOUND,
						"Record found in appl bene for remittacne :" + remitTrnx.getApplicationDocumentNo());
			}
		}
	...
	public Map<String, RemittanceAdditionalInstructionData> saveRemitnaceinstructionData(
			List<RemittanceTransaction> remitTrnxList) {
		Map<String, RemittanceAdditionalInstructionData> addInsData = new HashMap<>();

		for (RemittanceTransaction remitTrnx : remitTrnxList) {
			AdditionalInstructionData applInstrucData = addInstrDataRepository
					.findOne(remitTrnx.getApplicationDocumentNo());
			...
			if (applInstrucData != null) {
				...
				addInsData.put(applInstrucData.getDocumentFinanceYear() + "/" + applInstrucData.getDocumentNo(),
						remitAddData);

			} else {
				throw new GlobalException(JaxError.NO_RECORD_FOUND,
						"Record found in appl additional instruction  :" + remitTrnx.getApplicationDocumentNo());
			}
		}
	...
	public Map<String, RemittanceAml> saveRemittanceAml(List<RemittanceTransaction> remitTrnxList) {
		...
		for (RemittanceTransaction remitTrnx : remitTrnxList) {
			...
			if (applAml != null) {
				...
				amlList.put(remitTrnx.getApplicationFinanceYear() + "/" + remitTrnx.getApplicationDocumentNo(),
						remitAml);
			}
	...
	public BigDecimal getDeclarationReportAmount(String authType) {
		...
		BigDecimal declarationTotalamount = authParam.getAuthLimit() == null ? BigDecimal.ZERO
				: authParam.getAuthLimit();
	...
	public BigDecimal generateDocumentNumber(BigDecimal appCountryId, BigDecimal companyId, BigDecimal documentId,
			BigDecimal finYear, BigDecimal branchId) {
		Map<String, Object> output = applicationProcedureDao.getDocumentSeriality(appCountryId, companyId, documentId,
				finYear, ConstantDocument.A, branchId);
	...
	public TransferDto getTrasnferModeByBankServiceRule(RemittanceTransaction remitTrnx) {
		...
		if (mapBankServiceRule != null) {
			transferMode = mapBankServiceRule.get("P_TRANSFER_MODE") == null ? ""
					: mapBankServiceRule.get("P_TRANSFER_MODE").toString();
			transferModeId = mapBankServiceRule.get("TRANSFER_MODE_ID") == null ? BigDecimal.ZERO
					: (BigDecimal) mapBankServiceRule.get("P_TRANSFER_MODE_ID");
